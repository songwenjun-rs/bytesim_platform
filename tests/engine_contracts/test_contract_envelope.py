...
from generated.engine_contracts import (
    CoverageEnvelope,
    HardwareScope,
    ParallelismRange,
)
from generated.selector import envelope_covers, validate_envelope_intervals


def _surrogate_envelope() -> CoverageEnvelope:
    """Mirrors the planned RFC §4.2 surrogate-analytical envelope."""
    return CoverageEnvelope(
        model_families=["transformer-dense", "transformer-moe"],
...
def _narrow_envelope() -> CoverageEnvelope:
    """A deliberately narrow coverage envelope used to exercise the
    miss-reason path of `envelope_covers()`. Restricts model_families,
    parallelism degrees, GPU count, and overlap algorithms to a single
    legal combination so most test inputs miss on at least one axis."""
    return CoverageEnvelope(
        model_families=["transformer-dense"],
        parallelism=ParallelismRange(
...
class TestEnvelopeSchema:
    def test_realistic_surrogate_envelope_validates(self) -> None:
        ...
        assert env.model_families[0] == "transformer-dense"
    ...
    def test_inverted_parallelism_interval_rejected(self) -> None:
        ...
        # moved out of the Pydantic model_validator and into the registry's
        # register handler via validate_envelope_intervals(). Schema-time
        # construction now passes; the runtime check raises ValueError.
        env = CoverageEnvelope(
            ...
            hardware=HardwareScope(gpu_models=["B200"], fabric=["nvlink"],
                                    scale_gpus=(8, 8)),
            quant=["FP8"], modes=["training"],
        )
        with pytest.raises(ValueError) as ei:
            validate_envelope_intervals(env)
    ...
    def test_empty_model_families_rejected(self) -> None:
        with pytest.raises(ValidationError):
            CoverageEnvelope(
                model_families=[],  # min_length=1
                parallelism=_surrogate_envelope().parallelism,
    ...
    def test_unknown_quant_rejected(self) -> None:
        with pytest.raises(ValidationError):
            CoverageEnvelope(
                ...
                parallelism=_surrogate_envelope().parallelism,
                hardware=_surrogate_envelope().hardware,
                quant=["FP2"],  # not in Quant literal
                modes=["training"],
            )
...
class TestEnvelopeCovers:
    ...
    def test_surrogate_covers_typical_llama_training(self) -> None:
        ok, misses = envelope_covers(
            _surrogate_envelope(),
            model_family="transformer-dense", mode="training", quant="FP8",
    ...
    def test_surrogate_covers_moe_inference(self) -> None:
        ok, _ = envelope_covers(
            _surrogate_envelope(),
            model_family="transformer-moe", mode="inference", quant="FP8",
    ...
    # --- narrow envelope (exercises miss-reason paths) ------------------

    def test_narrow_covers_small_dense_training(self) -> None:
        ok, _ = envelope_covers(
            _narrow_envelope(),
            model_family="transformer-dense", mode="training", quant="FP8",
    ...
    def test_narrow_rejects_moe_workload(self) -> None:
        ok, misses = envelope_covers(
            _narrow_envelope(),
            model_family="transformer-moe",  # not declared
        ...
        assert not ok
        assert any(m.field == "model_family" for m in misses)

    def test_narrow_rejects_oversize_cluster(self) -> None:
        ok, misses = envelope_covers(
            _narrow_envelope(),
            model_family="transformer-dense", mode="training", quant="FP8",
    ...
    def test_narrow_rejects_pp_above_one(self) -> None:
        ok, misses = envelope_covers(
            _narrow_envelope(),
            model_family="transformer-dense", mode="training", quant="FP8",
            gpu_model="H200", gpu_count=8,
    ...
    def test_narrow_rejects_unknown_overlap(self) -> None:
        ok, misses = envelope_covers(
            _narrow_envelope(),
            model_family="transformer-dense", mode="training", quant="FP8",
            gpu_model="H200", gpu_count=8,
    ...
    def test_misses_collected_not_short_circuited(self) -> None:
        ...
        assert {"model_family", "mode", "quant"}.issubset(fields)
    ...
    def test_fabric_unknown_reported(self) -> None:
        ok, misses = envelope_covers(
            ...
            TP=8, PP=1, EP=1, CP=1,
            recompute="selective", overlap="1F1B",
            fabric_kinds=["roce"],  # narrow envelope only declares nvlink + infiniband
        )
        assert not ok
