...
9. select 阶段：
   ...
   c. POST tco_engine_svc /v1/tco/compute (persist=true) → bs_tco_breakdown
...
- 调用 tco_engine_svc 拆 TCO（best-effort）
...
| select | 90–100% | ≈0.5s | mark is_best · 写 4 个 artifact · best-effort 调 tco-engine（P1.4 rename，旧名 attribution） |
